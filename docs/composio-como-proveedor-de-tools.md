...
- **Fase 3 (UI): COMPLETA (piloto Gmail).** Modelo UX nuevo — **el panel `AgentToolsPanel` está MUERTO** (no se renderiza); las tools se crean desde la **tab Tareas** (árbol) y las **conexiones** se gestionan en **Integraciones**. Composio se partió en esos dos lugares:
